...
class sol_dystoneVisitor(SolidityVisitor):
    ...
    def visitPragmaDirective(self, ctx: SolidityParser.PragmaDirectiveContext):
        str = ctx.getChild(0).__str__() + " " + self.visitVersionPragma(ctx.pragma()) + ctx.getChild(
            2).__str__() + "\n\n"
        return str
    ...
    def visitContractDefinition(self, ctx: SolidityParser.ContractDefinitionContext):
        cp = ""
        str = ctx.getChild(0).__str__() + " " + self.visitIdentifier(ctx.identifier()) + " " + ctx.getChild(
            2).__str__() + "\n"
    ...
    def visitStateVariableDeclaration(self, ctx: SolidityParser.StateVariableDeclarationContext):
        ...
        if ctx.identifier() is ctx.getChild(ctx.getChildCount() - 2):
            ...
            res += ctx.getChild(ctx.getChildCount() - 3).__str__() + ctx.visit(ctx.expression()) + ctx.getChild(
                ctx.getChildCount() - 1).__str__()
    ...
    def visitConstructorDefinition(self, ctx: SolidityParser.ConstructorDefinitionContext):
        ...
        res = "@c" + str(self.fcnt) + "\n\t"
        self.fcnt += 1
        res += ctx.getChild(0).__str__() + self.visitParameterList(ctx.parameterList()) + self.visitModifierList(
            ctx.modifierList()) + self.visitBlock(ctx.block())
        return res
    ...
    def visitModifier(self, ctx: SolidityParser.ModifierContext):
        # 바로 아래로 내려보내기
        if ctx.getChild(0).__str__().__eq__('public') or ctx.getChild(0).__str__().__eq__("internal") or ctx.getChild(
                0).__str__().__eq__("private"):
    ...
    def visitMapping(self, ctx: SolidityParser.MappingContext):
        ...
        if isparent:  # parent가 업데이트 된경우 사용
            ...
            self.count += 1
            res += ctx.getChild(atindex - 1).__str__() + " " + self.visitAnnotatedTypeName(
                ctx.annotatedTypeName()) + "@_t" + str(self.parent[self.count])
            # 인덱스 정보 추가하기
            ...
            self.count += 1
            res += ctx.getChild(atindex - 1).__str__() + " " + self.visitAnnotatedTypeName(
                ctx.annotatedTypeName()) + "@_t" + str(self.count)
            # 인덱스 정보 추가하기
            self.rindex[name] = self.count
        self.count += 1
        res += ctx.getChild(ctx.getChildCount() - 1).__str__()
    ...
    def visitIfStatement(self, ctx: SolidityParser.IfStatementContext):
        ...
        str = ctx.getChild(0).__str__() + ctx.getChild(1).__str__() + self.visit(ctx.expression()) + ctx.getChild(
            3).__str__() + self.visitStatement(ctx.statement(0)) + "     "
    ...
    def visitWhileStatement(self, ctx: SolidityParser.WhileStatementContext):
        ...
        str = ctx.getChild(0).__str__() + " " + ctx.getChild(1).__str__() + self.visit(ctx.expression()) + ctx.getChild(
            3).__str__() + self.visitStatement(ctx.statement())
        return str
    ...
    def visitDoWhileStatement(self, ctx: SolidityParser.DoWhileStatementContext):
        str = ctx.getChild(0).__str__() + self.visitStatement(ctx.statement()) + "    " + ctx.getChild(
            2).__str__() + ctx.getChild(3).__str__() + self.visit(ctx.expression()) + ctx.getChild(
            5).__str__() + ctx.getChild(6).__str__()
        return str
    ...
    def visitIteExpr(self, ctx: SolidityParser.IteExprContext):
        ...
        str = self.visit(ctx.expression(0)) + " " + ctx.getChild(1).__str__() + " " + self.visit(
            ctx.expression(1)) + " " + ctx.getChild(3).__str__() + " " + self.visit(ctx.expression(2))
        return str
    ...
    def visitIndexExpr(self, ctx: SolidityParser.IndexExprContext):
        ...
        str = self.visit(ctx.expression(0)) + ctx.getChild(1).__str__() + self.visit(ctx.expression(1)) + ctx.getChild(
            3).__str__()
        return str
            ...
            # _reveal이 없으면, 평소대로 _reveal추가
                ...
                if isparent:
                    ...
                    if pc == 0 or pc == 1:
                        ...
                        res += " " + ctx.getChild(1).__str__() + " _reveal(" + rex + ", @_t" + str(
            ...
            # reveal 사이의 변수가 아니라면, _reveal 추가
                ...
                if isparent:
                    ...
                    if pc == 0 or pc == 1:  # me/all일경우 기존 번호 태깅
                        res = self.visit(ctx.expression(0)) + " " + ctx.getChild(
                            1).__str__() + " _reveal(" + rex + ", @_t" + str(self.count) + ")"
                    else:
                        res = self.visit(ctx.expression(0)) + " " + ctx.getChild(
                            1).__str__() + " _reveal(" + rex + ", @_t" + str(pc) + ")"
                else:
                    res = self.visit(ctx.expression(0)) + " " + ctx.getChild(
                        1).__str__() + " _reveal(" + rex + ", @_t" + str(self.count) + ")"
                self.count += 1
                ...
                if len(self.parent):
                    ...
                    if pc == 0 or pc == 1:
                        res += ctx.getChild(1).__str__() + "_reveal(" + fa + ", @_t" + str(
                            self.count) + ")" + ctx.getChild(3).__str__()
                    else:
                        res += ctx.getChild(1).__str__() + "_reveal(" + fa + ", @_t" + str(pc) + ")" + ctx.getChild(
                            3).__str__()
                else:
                    res += ctx.getChild(1).__str__() + "_reveal(" + fa + ", @_t" + str(self.count) + ")" + ctx.getChild(
                        3).__str__()
                self.isUse = True
                self.count += 1
                flag = False
    ...
    def visitPrimitiveCastExpr(self, ctx: SolidityParser.PrimitiveCastExprContext):
        ...
        str = self.visitElementaryTypeName(ctx.elementaryTypeName()) + ctx.getChild(1).__str__() + self.visit(
            ctx.expression()) + ctx.getChild(3).__str__()
        return str
